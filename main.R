...
listOfAuditFiltersClean <- c("SBP < 90", "Death at 30 days", "ISS > 15 and no team activation", 
                             "Massive transfusion", "ISS > 15 and not in ICU", "GCS < 9 and not intubated",
...
#makeTableFour <- make_table_four(missingValues)
...
tableOne <- tbl_summary(tableOneData, 
  ...
  modify_caption("Demographic and Clinical Characteristics of patients screened for OFI.") %>%
  modify_header(label = "",
                stat_1 = "**No**, (N = {n})",
                stat_2 = "**Yes**, (N = {n})") %>%
  ...
  bold_labels()
############TABLE ONE#######################
tableThree <- gt(tableOfCalculatedData1) %>% 
  cols_label(Number = "(N)",
             Specificity = "Specificity (%)",
             Sensitivity = "Sensitivity (%)") %>%
  cols_align(align = "left") %>%
  tab_caption(caption = md("Table 2: Performance of auditfilter in predicting OFI")) %>%
...
tableTwo <- gt(tableFourData) %>%
  ...
  cols_align(align = "left") %>%
  tab_caption("Table 4. Audit filters missing values") %>%
  tab_source_note("Definition of abbreviations: 
  OFI = Opportunity for Improvement; 
  ED = Emergency Department; 
  GCS = Glascow Coma Scale; 
